...
import Footer from "../component/Footer";
  ...
  const DaftarInstrumenPage = () => {
    ...
    const getFilterCategory = (instrumen) => {
      const kode = instrumen.Kode?.trim();
      const noPeralatan = instrumen.NoPeralatan?.trim();

      // Debug log untuk melihat data yang diproses
      console.log(`Processing instrument: Kode="${kode}", NoPeralatan="${noPeralatan}"`);

      // Prioritas untuk kategori khusus dari NoPeralatan
      if (noPeralatan) {
        if (noPeralatan.includes("AWOS")) return "AWOS";
        if (noPeralatan.includes("PC Display") || 
            noPeralatan.includes("PC OBS") || 
            noPeralatan.includes("PC AFTN") || 
            noPeralatan.includes("PC FCT") || 
            noPeralatan.includes("PC National Digital Forcasting") || 
            noPeralatan.includes("PC Maritim")) {
          return "PC";
        }
      }
      
      // Ekstrak kategori dari kode
      if (!kode) {
        console.log('No kode found, returning Lain-lain');
        return 'Lain-lain';
      }

      // Coba berbagai pemisah yang mungkin digunakan
      let category = '';
      
      if (kode.includes('_')) {
        category = kode.split('_')[0]?.trim();
      } else if (kode.includes('-')) {
        category = kode.split('-')[0]?.trim();
      } else if (kode.includes('.')) {
        category = kode.split('.')[0]?.trim();
      } else if (kode.includes(' ')) {
        category = kode.split(' ')[0]?.trim();
      } else {
        // Jika tidak ada pemisah, ambil 2-4 karakter pertama
        category = kode.substring(0, Math.min(4, kode.length));
      }
      
      if (category) {
        const finalCategory = category.toUpperCase();
        console.log(`Kode "${kode}" -> Category "${finalCategory}"`);
        return finalCategory;
      }
      
      console.log(`No category found for kode "${kode}", returning Lain-lain`);
      return 'Lain-lain';
    };
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          // Buat set kategori dengan logging untuk debug
          const categories = new Set();
          data.forEach(instrumen => {
            const category = getFilterCategory(instrumen);
            if (category && category !== 'Lain-lain') {
              categories.add(category);
              // Debug log untuk melihat kategori yang dibuat
              console.log(`Kode: ${instrumen.Kode} -> Kategori: ${category}`);
            }
          });
          
          // Tambahkan 'Lain-lain' jika ada data yang masuk kategori tersebut
          const lainLainCount = data.filter(instrumen => getFilterCategory(instrumen) === 'Lain-lain').length;
          if (lainLainCount > 0) {
    ...
    const filteredInstrumenData = useMemo(() => {
      ...
      const filtered = instrumenData.filter(instrumen => {
        const category = getFilterCategory(instrumen);
        const matches = category === selectedKodeCategory;
        
        // Debug logging
        if (matches) {
          console.log(`Match found - Kode: ${instrumen.Kode}, Category: ${category}, Selected: ${selectedKodeCategory}`);
        }
    ...
    // Tambahan: Fungsi helper untuk debugging kategori
    const debugFilterData = () => {
      console.log('=== DEBUG FILTER DATA ===');
      console.log(`Total instruments: ${instrumenData.length}`);
      console.log(`Selected category: "${selectedKodeCategory}"`);
      console.log(`Filtered results: ${filteredInstrumenData.length}`);
      
      // Group by category untuk melihat distribusi
      const categoryCount = {};
      instrumenData.forEach(instrumen => {
        const category = getFilterCategory(instrumen);
        categoryCount[category] = (categoryCount[category] || 0) + 1;
      });
      
      console.log('Category distribution:', categoryCount);
      console.log('Available categories:', kodeCategoryOptions);
      
      // Show sample of filtered data
      console.log('Sample filtered data:', filteredInstrumenData.slice(0, 3));
      console.log('========================');
    };
  ...
  const renderEditableField = (label, fieldName, value, type = 'text') => {
    ...
    // Debug log untuk field Drift
    if (fieldName === 'Drift') {
      console.log(`Drift field - isEditing: ${isCurrentlyEditing}, value: "${value}", editData: "${editData[fieldName]}"`);
    }
      ...
      <div className="flex flex-col min-h-screen">
        ...
        <div className="flex flex-1 relative">
          ...
          <main className="flex-1 p-4 sm:p-6 lg:p-8">
            ...
            {/* LETAKKAN INDIKATOR HASIL FILTER DI SINI */}
            {selectedKodeCategory && (
              <div className="text-center text-sm text-gray-600 mb-4">
                Menampilkan {filteredInstrumenData.length} instrumen untuk kategori: 
                <span className="font-semibold text-blue-600 ml-1">{selectedKodeCategory}</span>
              </div>
            )}
                ...
                <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="grid grid-cols-1 gap-2 sm:gap-3 text-gray-700 text-xs sm:text-sm md:text-base">
                      ...
                      {renderEditableField('Drift', 'Drift', selectedInstrumen.Unit)}

...
function App() {
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-red-100 p-4 rounded-lg shadow-lg m-4">
        <h2 className="text-2xl font-bold text-red-700 mb-2">Terjadi Kesalahan!</h2>
        <p className="text-red-600 mb-4">{error}</p>
        <p className="text-red-500 text-sm">Pastikan URL Google Apps Script Anda benar, sudah di-deploy, dan sheet 'TEKNISI' ada serta berisi data.</p>
      </div>
    );
  }

  return (
    // Menggunakan Tailwind untuk layout responsif dan styling
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 md:p-8 font-sans">
      ...
      <div className="max-w-6xl mx-auto bg-white p-6 rounded-lg shadow-xl">
        <h1 className="text-3xl font-bold text-center text-gray-800 mb-8">Profil Teknisi</h1>

        {/* Dropdown untuk memilih nama pegawai */}
        <div className="mb-8 flex flex-col sm:flex-row items-center justify-center gap-4">
          <label htmlFor="select-pegawai" className="text-lg font-medium text-gray-700">Pilih Teknisi:</label>
          <select
            id="select-pegawai"
        ...
        {selectedPegawai ? (
              ...
              {selectedPegawai.FotoURL ? (
                <img
                  ...
                  onError={(e) => { // Penanganan error jika gambar tidak bisa dimuat
                    ...
                    e.target.src = "https://placehold.co/262x262/E2E8F0/64748B?text=Tidak+Ada+Foto"; // Placeholder
                  }}
                />
              ) : (
                <img src={user} alt="Default User" className="max-w-full h-auto rounded-lg"/>
              )}
            </div>
            {/* Bagian Kiri: Detail Pegawai */}
            <div className="flex-1">
              <h2 className="text-2xl font-semibold text-blue-800 mb-4 text-center md:text-left">Nama Teknisi: {selectedPegawai.NAMA}</h2>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-gray-700">
                <div className="detail-item">
                  <span className="font-medium">NIP:</span> {selectedPegawai.NIP}
                </div>
                {/* Pangkat / Gol. - Tambahkan col-span-full */}
                <div className="detail-item col-span-full">
                  <span className="font-medium">Pangkat / Gol.:</span> {selectedPegawai["PANGKAT / GOL"]}
                </div>
                <div className="detail-item">
                  <span className="font-medium">Fungsional:</span> {selectedPegawai.FUNGSIONAL}
                </div>
                {/* Pendidikan Terakhir - Tambahkan col-span-full */}
                <div className="detail-item col-span-full">
                  <span className="font-medium">Pendidikan Terakhir:</span> {selectedPegawai["PENDIDIKAN TERAKHIR"]}
                </div>
                <div className="detail-item col-span-full">
                  <span className="font-medium">Diklat/Workshop/Temu Teknisi:</span> {selectedPegawai["DIKLAT/WORKSHOP/TEMU TEKNISI"]}
                </div>
                <div className="detail-item">
                  <span className="font-medium">Tugas:</span> {selectedPegawai.TUGAS}
                </div>
                {/* Keterangan Tambahan - Tambahkan col-span-full */}
                <div className="detail-item col-span-full">
                  <span className="font-medium">Keterangan:</span> {selectedPegawai.KETERANGAN}
                </div>
              </div>
              <button
          ...
          pegawaiData.length === 0 ? (
            <p className="text-center text-gray-600 mt-4">Tidak ada data pegawai yang ditemukan.</p>
          ) : (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {pegawaiData.map((pegawai, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{pegawai.NAMA}</td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{pegawai.NIP}</td>
